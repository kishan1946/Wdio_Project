...
export const config: Options.Testrunner = {
    ...
    beforeStep: function (step, scenario, context) {
        ...
        const idAndSeverity = scenario.name.split(" ")[0].split(":");
        if (process.env.BROWSER === "CHROME" || process.env.BROWSER === "FIREFOX" || process.env.BROWSER === "EDGE") {
            ENV_NAME = process.env.BROWSER;
            ENV_VALUE = process.env.BROWSER_VERSION;
        }
        let SEVERITY= idAndSeverity[1].trim();
        allure.addEnvironment(ENV_NAME, ENV_VALUE);
